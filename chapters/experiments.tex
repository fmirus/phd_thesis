\chapter{Closed-loop neuromorphic systems}%
\label{chap:closed_loop_neuromorphic_systems}
...
Figure \ref{subfig:omni_bot_out_of_order_net_behavior} shows an example of actual \ac{DVS} input data from the embedded tracking algorithm as well as the decoded output of the network’s subcomponents activity: during the first \SI{5}{\second} the stimuli are in correct descending order from left to right in the \ac{DVS}-image (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} a), so the minimum pairwise difference is non-negative (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} b and \ref{subfig:omni_bot_out_of_order_net_behavior} c). 
In the interval \SIrange{5}{15}{\second}, the \SI{250}{\hertz} stimulus is put between the \SI{150}{\hertz} and \SI{200}{\hertz} stimuli, so now the sequence is out of order and the \SI{250}{\hertz} frequency is detected by the odd ensemble (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} d) while the evidence networks integrate accordingly (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} f-h). 
Starting from around \SI{15}{\second} the \SI{250}{\hertz} and \SI{350}{\hertz} stimuli are interchanged and the network's outputs change accordingly (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} a-d).
However, the evidence networks - as desired - still keep the information about the old target until a forget mechanism (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} e) is triggered in the interval \SIrange{20}{25}{\second} allowing the evidence networks to recover for new input (Fig. \ref{subfig:omni_bot_out_of_order_net_behavior} f-h).
...
The \emph{TargetInfo} ensemble encodes sensory information ($x$- and $y$-position in the image, as well as radius and track-certainty for each \ac{DVS}) of the target object (coming from the evidence sub-network in the out of order network), which serves as input for the low-level behaviors.
The has-grabbed ensemble keeps the information once the object was grabbed in memory to indicate this task finished successfully.
Figure \ref{subfig:omni_bot_grab_net_behavior} shows the activation levels of the high- and low-level behaviors (Fig. \ref{subfig:omni_bot_grab_net_behavior} a and \ref{subfig:omni_bot_grab_net_behavior} b) as well as the sensory information the behaviors make use of, namely tracking certainty and disparity and $x$-position of the target object in the arm retina (Fig. \ref{subfig:omni_bot_grab_net_behavior} c and \ref{subfig:omni_bot_grab_net_behavior} d respectively). 
...
Figure \ref{fig:omni_bot_example_run} illustrates the most important stages of one example run while Figure \ref{subfig:omni_bot_grab_and_sort_net_behavior} gives a visualization of the decoded output of the network’s components: once the Out of Order network detected the target object (roughly the first 5 s), the Grab behavior is activated to find grasp the target object (Fig. \ref{fig:omni_bot_example_run} a-c, Fig. \ref{subfig:omni_bot_grab_and_sort_net_behavior}, t = 8s - 26s). 
...
Once the robot grabbed the target object, the Basal Ganglia activates the Hold and Move Side behavior \ref{holdmoveside} (Fig. \ref{fig:omni_bot_example_run}d-e, Fig. \ref{subfig:omni_bot_grab_and_sort_net_behavior}, t = 26s - 39s). 

The main sensory input in this phase is the $x$-position of the neighbor objects (Fig. \ref{subfig:omni_bot_grab_and_sort_net_behavior} e). 
A clear indicator for successful pick-up is the decrease of certainty in the base-retinas (Fig. \ref{subfig:omni_bot_grab_and_sort_net_behavior} d). 
Once the robot reached its goal position between the neighboring objects, the Put Down behavior \ref{putdown} is activated (Fig. \ref{fig:omni_bot_example_run} f-g). 
