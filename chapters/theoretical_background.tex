...
\end{figure}
\todo{in fig \ref{fig:cosine_dist} remove figure title in python, make labels larger for better readability}
...
\begin{defn}
	...
	\todo[inline]{maybe use two different $\epsilon$ for $2$ or $3$ for definition of similar and highly similar?}
\end{defn}
...
\begin{lemma}
	...
	Let $\mathcal{U}$ be the set of unitary vectors in the $D$-dimensional \ac{SPA} $\mathcal{S}(D)$. The following statements hold
 	\begin{enumerate}[label=\roman*]
        ...
        \item The product under circular convolution of two unitary vectors is unitary, i.e.\ $u_{1} \varoast u_{2} \in \mathcal{U} $ for $u_{1}, u_{2} \in mathcal{U}.$
...
\begin{proof}
    ...
    Let $u_{1}, u_{2} \in mathcal{U}$ be unitary vectors, then we have
    \[
        \ac{DFT} \left(u_1 \varoast u_2\right) = \ac{DFT} \left( \ac{IDFT} \left( \ac{DFT} \left(u_1\right) \odot \ac{DFT} \left(u_2\right)\right)\right) =  \ac{DFT} \left(u_1\right) \odot \ac{DFT} \left(u_2\right).
    \]
    Writing both $\ac{DFT}_j(u_1) = r_{1j}\exp\left(\varphi_{1j}\right)$ and $\ac{DFT}_j(u_2) = r_{2j}\exp\left(\varphi_{2j}\right)$ in polar coordinates, we get 
    \[
        \ac{DFT}_j \left(u_1 \varoast u_2\right) = r_{1j} \cdot r_{2j} \cdot \exp \left(\varphi_{1j} + \varphi_{2j}\right) = 1 \cdot \exp \left(\varphi_{1j} + \varphi_{2j}\right),
    \]
    as we have $r_{1j}=1$ and $r_{2j}=1$ for all magnitudes since both $u_1$ and $u_2$ are unitary vectors.
...
\end{figure}
\todo{check if fig. \ref{fig:nef_dynamics} can be beautified}
...
\todo{include phrase regarding preprocessing as described in \cite{Gallant2013} and refer to it later in this section}
...
%\begin{figure}[t!]
%	\centering
%	\includegraphics[width=0.6\textwidth]{imgs/SPA_constructed_vocab_sim.png}
%	\caption{Cosine similarities in a simple, manually engineered vector vocabulary of dimension $256$.}
%	\label{fig:constructed_vocab_sims}
%\end{figure}
...
% 			\subfloat[\label{subfig:cnn_arch}]{%
% 				\includegraphics[width=0.7\textwidth, height=6cm]{imgs/CNNArchitecture.jpg}
% 			}
% 			\subfloat[\label{subfig:word2vec}]{%
% 				\centering
% 				\begin{tikzpicture}[%
% 				auto,
% 				scale=5,
...
% 				}
% 			}
% 	\caption{bla bla bla}\label{fig:vector vocabs2}
% \end{figure}
...
The most successful approaches such as \textit{word2vec} \cite{Mikolov2013a, Mikolov2013} or \textit{\ac{GloVe}} \cite{Pennington2014} typically use large corpora of text as input data and are trained in unsupervised fashion based on co-occurence statistics of words.
...
The underlying assumption is that words sharing many contexts are similar to each other, such that automated training with the aforementioned objective will produce good word embeddings.
...
\todo[inline]{include raven matrix example \cite{Rasmussen2011}}

\subsection{Implementation in \aclp{SNN}}%
\label{subsec:implementation_in_snns}


\todo[inline]{explain how modelling with VSAs works referring to neural modelling section and explain implementation in SNNs}
\section{Summary}
