...
\begin{ex} \aclp{VSA}
	...
	\begin{enumerate}
		...
		\]
		From the convolution theorem (\todo{insert citation here!})we know, that we can calculate the circular convolution of any two vectors $v, w \in V_{D}(N)$ by
		\[
...
We have already seen, that we can use the \acf{DFT} and \acf{IDFT} to calculate circular convolution efficiently by element-wise multiplication (this follows from the convolution theorem) in the frequency domain, i.e.
\[
	v \varoast w = \ac{IDFT}\left(\ac{DFT}(v) \odot \ac{DFT}(w) \right).
\]
...
The relation between the vector norm and the magnitudes in the frequency domain is given by Parseval's theorem, which states
\[
...
\todo{briefly explain other applications like natural langua processing, word2vec etc.}
