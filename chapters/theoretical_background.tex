...
\begin{figure}[t]
    ...
    \label{fig:cosine_dist/distributions_cosine_sims}
\end{figure}
Figure~\ref{fig:cosine_dist}, which shows the probability distributions of cosine similarity for different vector dimensions, illustrates this result.
...
\todo[inline]{explain implementation in SNNs}
$W = \frac{1}{ \sqrt{D-1}} \left(\zeta_{D} ^{i\cdot j}\right)_{i,j=0}^{D-1}$
...
Finally, we have introduced the basic principles of cognitive modeling with \acp{VSA} and how structured representations can be built from atomic vectors.
