...
Furthermore, there is an interest in publicly available data-sets which not only enable competition between researchers bout also allow for transparent benchmarking of approaches against each other.
Data sets for image classification such as the \ac{MNIST} data set for hand-written digits \cite{LeCun1998} , the \ac{GTSRB} for traffic sign recognition \cite{Stallkamp2012} and the ImageNet data set \cite{Deng2009} provided researchers with large amounts of labeled image data facilitating significant progress.
One of the first data sets publicly available on a larger scale for an automotive context and encouraging competition is the KITTI vision benchmark suite \cite{Geiger2013a}, which was recorded using the Annieway vehicle prototype \cite{Annieway} in urban environments in the region of the German city Karlsruhe.
...
The Cityscapes data set \cite{Cordts2016} provides high-definition images recorded from a driving vehicle with accurate pixel-wise labels for semantic segmentation.
A comparable data set for semantic image segmentation in a broader context (i.e., not restricted to automotive context) is the 
\cite{COCO} data set \cite{Lin2014}.
